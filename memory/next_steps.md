...
项目刚刚启动，仓库已经加入记忆系统，Qt 环境可用，第一个 Qt Widgets 程序已经可以构建和运行，并已推送到 GitHub。当前学习状态是：已经理解第一个 `src\main.cpp` 的意思，包括 `QApplication`、`QLabel`、`show()` 和 `app.exec()`。目前已经把 `QLabel` 小窗口改成登录窗口雏形，并让登录按钮点击后通过信号槽更新窗口标题。
